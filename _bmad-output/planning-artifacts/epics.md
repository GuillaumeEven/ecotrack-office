...
FR11: The floor map is auto-generated by the Angular frontend from the structured data (rooms, desks) returned by the API; no SVG file upload is required
FR12: The position of each room tile and desk marker on the map is determined automatically by the Angular map component based on their order in the API response; no manual anchor positioning is required
...
FR18: An Employee can view a detail card for any desk or room (equipment, capacity, floor, current status)
...
FR35: An Organization Admin can view total reservations per room for the current day and current week
FR36: An Organization Admin can view occupancy rate (confirmed check-ins vs. total reservations) per room per day
FR37: The system generates a room consolidation suggestion when daily attendance falls below a configurable threshold, identifying rooms to activate and rooms to shut down
FR38: *(Nice-to-have — post-MVP)* An Organization Admin can send a targeted notification to Employees booked in a room recommended for shutdown
...
NFR15: Database schema supports multiple buildings from day one; no hardcoded limits on floors, rooms, or desks
...
UX-DR2: Color-coded resource status overlay on the floor map (green = available, orange = active room / energy nudge, red = unavailable/incident)
UX-DR3: *(Nice-to-have — post-MVP)* Room activity heat overlay nudging employees toward energy-efficient clustering without mandating it
...
UX-DR6: Desk/room detail card with equipment, capacity, floor, and current status
...
UX-DR10: Admin dashboard with daily/weekly occupancy summary, open incidents count, and room consolidation suggestion card
...
FR11: Epic 2 — Map auto-generated from rooms/desks API (no SVG upload)
FR12: Epic 2 — Room/desk positions auto-calculated by Angular map component (no anchor association)
...
FR35: Epic 4 — Occupancy dashboard (reservations per room)
FR36: Epic 4 — Check-in rate per room per day
FR37: Epic 4 — Room consolidation suggestion engine
FR38: *(post-MVP)* Epic 4 — Targeted employee notification (room shutdown)
...
> FR11 and FR12 reflect the auto-generated map approach (no SVG upload); the map is rendered client-side from the rooms/desks API.
...
Any user can report a resource incident; technicians are notified in real time via SSE; the Organization Admin has a full occupancy dashboard, energy/CO₂ savings estimates per closed room, room consolidation suggestions, and GDPR-compliant data governance tools.
...
**Then** `GET/POST /api/v1/desks` and `PUT/PATCH /api/v1/desks/{id}` manage desks (`ast_desks`: id, room_id FK, name, equipment JSON array, status ENUM(AVAILABLE/UNAVAILABLE), is_active)
...
### Story 2.3: Map Data API

As an Organization Admin,
I want the floor map to be automatically generated from the rooms and desks I create,
So that employees always see an up-to-date visual representation without any manual SVG work.

**Acceptance Criteria:**

**Given** I am authenticated as an Organization Admin and floors/rooms/desks exist
**When** the Angular map loads
**Then** `GET /api/v1/floors` returns the list of floors with their rooms; `GET /api/v1/floors/{id}/rooms` returns rooms for a floor with their desks and real-time status
**And** the Angular `BuildingMapComponent` generates an inline `<svg>` with `<rect>` tiles — one per room — positioned automatically based on their order in the API response
**And** no SVG file is stored server-side; the Spring backend stores only structured entities (FloorEntity, RoomEntity, DeskEntity)
**And** room ordering in the API response determines tile layout (left-to-right, top-to-bottom)
**And** the map data endpoint is accessible to all authenticated users (EMPLOYEE+)
...
**And** clicking an available room tile opens the `RoomDrillDownComponent` showing individual desk markers (green/orange/red) in a generated room-level tile layout
...
**And** each entry shows: resource name, floor, room, date, time slot, and current status
...
As an Organization Admin,
I want to view occupancy statistics per room for the current day and week including the check-in rate,
...
**Then** `GET /api/v1/analytics/occupancy?date={date}&granularity={day|week}` returns per-room: total reservations, confirmed check-ins, check-in rate (%), and peak hour
**And** the Angular `OccupancyDashboardComponent` provides a daily view (today) and weekly view (current week) switchable via tabs
**And** each room is displayed as a card showing its occupancy metrics and a simple bar visualization
...
### Story 4.5: Room Consolidation Suggestions
...
As an Organization Admin,
I want the system to suggest which rooms to activate or shut down on low-attendance days,
So that I can concentrate occupancy into fewer rooms and reduce energy waste.

**Acceptance Criteria:**

**Given** I am authenticated as an Organization Admin and daily attendance is below the configurable threshold
**When** I open the room consolidation panel
**Then** `GET /api/v1/analytics/consolidation-suggestions?date={date}` returns: rooms recommended to keep active, rooms to shut down, and count of affected employees per room
**And** the suggestion algorithm identifies rooms with the fewest reservations and recommends consolidating employees into the most occupied rooms
...
**And** the Angular `ConsolidationSuggestionsComponent` shows: recommended active rooms, rooms to close, and a list of employees booked per room (for manual outreach if needed)
...
**Then** `GET /api/v1/analytics/export?from={date}&to={date}&format=csv` returns a CSV with: date, room, total reservations, confirmed check-ins, check-in rate, open incidents — as a file download with `Content-Disposition: attachment; filename=occupancy-{from}-{to}.csv`
