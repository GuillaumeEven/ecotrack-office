...
editHistory:
  ...
  - date: '2026-05-19'
    changes: >
      Round 3 — Demo scope simplifications: JWT removed from MVP scope; authentication
      simplified to HTTP Basic Auth (plain credentials) for demo exercise; bcrypt deferred
      to Growth; JWT + refresh token strategy added as Growth nice-to-have.
      FR11 replaced: SVG floor plan upload removed; floor map now dynamically generated
      by Angular from stored floor/room/desk structure data (creation-order layout).
      FR12 removed: manual SVG anchor position association no longer needed;
      positions derived from creation order.
...
- Session-based authentication enforces role separation across all features
...
EcoTrack operates as a business-logic and analytics layer over a managed office building. It does not interface with physical control systems (HVAC, lighting hardware) in MVP scope. Domain constraints are non-trivial but do not involve life-safety or engineering authority requirements.
...
- **Authentication:** HTTP Basic Authentication (plain credentials) for MVP demo scope; no token expiry management. JWT stateless auth with refresh token strategy deferred to Growth.
...
- **Map rendering:** Floor map SVG dynamically generated by Angular from stored floor/room/desk structure data (creation-order layout); desk status overlaid as Angular components — no third-party mapping library dependency; no SVG file storage in backend
- **State management:** Angular services + RxJS observables; no NgRx required at MVP scale
- **Auth flow:** HTTP Basic Authentication for MVP demo; Angular route guards enforce role access; JWT with HttpOnly/Secure cookie storage deferred to Growth
...
|---|---|---|---|
| **Block 1** | Users & Security | User accounts (with company code), roles, session authentication | None — foundational |
...
| Risk | Mitigation |
|---|---|
| Block 2 SVG / map complexity | Week 1 spike to validate dynamic room-tile + desk rendering from stored structure data; list/table fallback always available |
...
- JWT stateless authentication with token expiry (8h) and refresh token strategy
...
Note: For SaaS Demo Mode the tenant-level admin role is `Organization Admin`. This role combines facilities management and tenant administration responsibilities (manage maps/assets, user accounts, tenant settings and demo plan).
...
- **FR2:** A registered user can log in with credentials and access the application according to their role
...
- **FR11:** The floor map for each floor is dynamically rendered by the frontend from stored floor, room, and desk structure data; room and desk layout positions are derived from creation order; no SVG file upload or storage in the backend
...
- **FR31:** The system delivers a push notification to all Technicians within 30 seconds of a new incident submission, with location and description
...
- **FR39:** An Organization Admin can export an occupancy summary report for a selected date range as CSV
...
- Passwords stored in plain text for MVP demo scope only; bcrypt (minimum cost factor 12) deferred to Growth
- All API endpoints require valid authentication except login and registration
- Role enforcement applied at API layer, not only in the frontend layer
...
- JWT token expiry, refresh token strategy, and HttpOnly/Secure cookie storage deferred to Growth
...
- OpenAPI/Swagger specification maintained as the binding frontend–backend contract
- Each of the four functional blocks deployable independently with its own data namespace
