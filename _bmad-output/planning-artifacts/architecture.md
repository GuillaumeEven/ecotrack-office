...
- Map: SVG floor plan **dynamically generated by Angular** from structured JSON (floors → rooms → desks);
  rectangular tiles rendered as inline `<svg><rect>` elements; no file upload, no third-party mapping library
...
2. Block 2 (assets): `ast_*` tables + floor/room/desk CRUD REST endpoints + map data API (Angular renders SVG from JSON)
...
```
Tables  : usr_users, ast_resources, ast_rooms, ast_desks, ast_floors, rsv_reservations, rsv_checkin_tokens, anl_incidents
Columns : user_id, created_at, is_active, room_id, auto_release_minutes
FKs     : user_id, room_id (never fk_user, never userId)
...
GET    /api/v1/rooms/{id}/occupancy-report
```
...
data: {"incidentId": 7, "resourceId": 42, "location": "Floor 3 - Room A"}
```
...
│           │   ├── assets.routes.ts
│           │   ├── floor-map/
│           │   │   ├── floor-map.component.ts      ← SVG généré dynamiquement depuis l'API (rooms/desks JSON)
│           │   │   ├── floor-map.component.spec.ts
│           │   │   ├── room-tile/
│           │   │   │   └── room-tile.component.ts    ← Tuile <rect> cliquable par salle
│           │   │   └── desk-marker/
│           │   │       └── desk-marker.component.ts  ← Pastille statut bureau (drill-down salle)
│           │   └── asset-admin/
...
└── backend/                        ← Spring Boot 3.5.x Maven project
    ...
    └── src/
        ...
        │   │   │   │   ├── ResourceEntity.java        ← table ast_resources (@MappedSuperclass)
        │   │   │   │   ├── RoomEntity.java            ← table ast_rooms (extends ResourceEntity)
        │   │   │   │   └── DeskEntity.java            ← table ast_desks (extends ResourceEntity)
        │   │   │   └── dto/
        │   │   │
...
| Employee self-service | `/api/v1/reservations/**`, `/api/v1/floors/**` | Yes | EMPLOYEE+ |
...
| FR8–FR12 Physical Assets | ✅ Block 2: Floor/Room/Desk controllers, ResourceEntity (@MappedSuperclass) → RoomEntity, DeskEntity |
| FR13–FR18 Floor Map | ✅ assets-mgmt/floor-map (SVG généré depuis JSON), room-tile, desk-marker, interval(30000) polling |
...
2. **UPLOAD_DIR sub-folder** — `UPLOAD_DIR/incident-photos/` for Block 4 incident photos. Configured via `UPLOAD_DIR` env var.
