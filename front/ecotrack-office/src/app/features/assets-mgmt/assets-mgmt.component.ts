...
import { DeskModel, FloorModel, ResourceStatusModel, RoomModel } from '@models/index.model';
...
@Component({
  ...
  styleUrls: ['./assets-mgmt.component.css'],
})
export class AssetsMgmtComponent implements OnInit {
  ...
  editingFloor: FloorModel | null = null;
  editingRoom: RoomModel | null = null;
  editingDesk: DeskModel | null = null;
  ...
  openDeleteDialog(type: 'floor' | 'room' | 'desk', id: number, name?: string) {
    ...
    if (type === 'floor') {
      ...
      this.deleteItemSummary =
        rooms.length > 0
          ? `Este piso contiene ${rooms.length} sala(s) y ${desks.length} escritorio(s) que también serán eliminados.`
          : '';
    } else if (type === 'room') {
      const desks = this.allDesks.filter((d) => d.roomId === id);
      this.deleteItemSummary =
        desks.length > 0
          ? `Esta sala contiene ${desks.length} escritorio(s) que también serán eliminados.`
          : '';
    } else {
      this.deleteItemSummary = '';
    }
      ...
      case 'desk':
        this.deskService.delete(this.deleteItemId).subscribe({
          next: () => {
            ...
            this.notificationService.success('Escritorio eliminado correctamente.');
            this.closeDeleteDialog();
            this.cdr.markForCheck();
          },
          error: () => {
            this.notificationService.error('Error al eliminar el escritorio.');
            this.cdr.markForCheck();
          },
        });
        break;
  ...
  openRoomDialog(room?: RoomModel | null) {
  ...
  openDeskDialog(desk?: DeskModel | null) {
